...
<head>
    <meta charset="UTF-8">
    <title>カート</title>
    <link rel="stylesheet" href="../css/shop.css"> <!-- minishop-->
</head>
...
<body>
  ...
  if(!isset($cart)){
    require_once __DIR__.'/../classes/cart.php'; //cart.phpを読み出し
    $cart = new Cart(); //Cartというオブジェクトを生成し、$cartに挿入
  }

  $cartItems = $cart->getItems($_SESSION['userId']); //だいたいココが原因　わからん
  if(empty($cartItems)){
